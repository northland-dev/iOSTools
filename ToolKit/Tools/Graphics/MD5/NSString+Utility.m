...
- (NSString *)getSHA1
{
	...
	if(hashBytes)
	{
		...
		CC_SHA1((void *)[srcData bytes], (unsigned int)[srcData length], hashBytes);
...
{
	// 分配MD5结果空间
	uint8_t *md5Bytes = malloc(CC_MD5_DIGEST_LENGTH * sizeof(uint8_t) / 2);
	if(md5Bytes)
	{
		...
		// 计算hash值
		NSData *srcData = [self dataUsingEncoding:NSUTF8StringEncoding];
		CC_MD5((void *)[srcData bytes], (unsigned int)[srcData length], md5Bytes);
