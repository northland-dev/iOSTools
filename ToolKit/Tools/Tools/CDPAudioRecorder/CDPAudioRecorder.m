...
-(void)playAudioWithUrl:(NSString *)url{
    ...
    if ([url isEqualToString:@""]||
        ...
        if ([_delegate respondsToSelector:@selector(playerFinishMethod:)]) {
            [_delegate playerFinishMethod:NO];
        }
        return;
    }
    //设置会话类别
...
//        [recorder averagePowerForChannel:0];
        ...
        NSLog(@"volumeMeters: %f",lowPassResults);
        
        [_delegate updateVolumeMeters:MAX(0, MIN(lowPassResults * 1.7, 1))];
    }
}
...
-(void)createTimer{
    if (_timer==nil) {
        _timer = [NSTimer timerWithTimeInterval:0.08 target:self selector:@selector(volumeMeters:) userInfo:nil repeats:YES];
